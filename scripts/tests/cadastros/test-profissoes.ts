...
const OUTPUT_FILE = path.join(__dirname, '../../reports/test-profissoes.html');

async function runTests() {
  console.log('🚀 Iniciando testes E2E: Profissoes CRUD');
  ...
  const profissao = mockGen.generateProfissao();
  const resultados = await runner.testCRUD('Profissao', 'profissao', profissao, {
    descricao: `Profissão Atualizada ${Date.now()}`
  });

  resultados.forEach(t => runner.logTestResult(t));

  const suite: TestSuite = {
    nome: 'Relatório E2E: Profissoes CRUD',
